...
function normalizeGraph(data) {
  ...
  const nodes = rawNodes.map((n, i) => ({
    ...
    x: 0.5 + 0.3 * Math.cos((2 * Math.PI * i) / Math.max(rawNodes.length, 1)),
    y: 0.5 + 0.3 * Math.sin((2 * Math.PI * i) / Math.max(rawNodes.length, 1)),
    vx: 0,
    vy: 0,
  }));
...
function useForceGraph(canvasRef, graph, onSelect) {
  ...
  useEffect(() => {
    ...
    const IDEAL_EDGE_LEN = 0.2;
    const SPRING_K = 0.008;
    const REPEL_K = 0.00035;
